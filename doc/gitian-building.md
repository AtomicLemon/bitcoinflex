...
*Setup instructions for a gitian build of BitcoinFlex using a Debian VM or physical system.*

Gitian is the deterministic build process that is used to build the BitcoinFlex
...
- [Building BitcoinFlex](#building-bitcoinflex)
...
Clone the git repositories for bitcoinflex and gitian and then checkout the bitcoinflex version that you want to build.

```bash
git clone https://github.com/devrandom/gitian-builder.git
git clone https://github.com/bitcoinflex-crypto/bitcoinflex.git
cd bitcoinflex
...
Follow the instructions in [doc/release-process.md](release-process.md) in the bitcoinflex repository
under 'Fetch and build inputs' to install sources which require manual intervention. Also follow
the next step: 'Seed the Gitian sources cache', which will fetch all necessary source files allowing
for gitian to work offline.

Building BitcoinFlex
----------------

To build BitcoinFlex (for Linux, OSX and Windows) just follow the steps under 'perform
gitian builds' in [doc/release-process.md](release-process.md) in the bitcoinflex repository.
...
```bash
    Initialized empty Git repository in /home/debian/gitian-builder/inputs/bitcoinflex/.git/
    remote: Reusing existing pack: 35606, done.
    remote: Total 35606 (delta 0), reused 0 (delta 0)
    Receiving objects: 100% (35606/35606), 26.52 MiB | 4.28 MiB/s, done.
    Resolving deltas: 100% (25724/25724), done.
    From https://github.com/bitcoinflex-crypto/bitcoinflex
...
For example:
```bash
URL=https://github.com/crowning-/bitcoinflex.git
COMMIT=b616fb8ef0d49a919b72b0388b091aaec5849b96
./bin/gbuild --commit bitcoinflex=${COMMIT} --url bitcoinflex=${URL} ../bitcoinflex/contrib/gitian-descriptors/gitian-linux.yml
./bin/gbuild --commit bitcoinflex=${COMMIT} --url bitcoinflex=${URL} ../bitcoinflex/contrib/gitian-descriptors/gitian-win.yml
./bin/gbuild --commit bitcoinflex=${COMMIT} --url bitcoinflex=${URL} ../bitcoinflex/contrib/gitian-descriptors/gitian-osx.yml
```
...
```bash
    gpg --detach-sign ${VERSION}-linux/${SIGNER}/bitcoinflex-build.assert
    gpg --detach-sign ${VERSION}-win/${SIGNER}/bitcoinflex-build.assert
    gpg --detach-sign ${VERSION}-osx/${SIGNER}/bitcoinflex-build.assert
```
...
[bitcoinflex/gitian.sigs](https://github.com/bitcoinflex-crypto/gitian.sigs/) repository, or if that's not possible to create a pull
request.
