...
CzBCXWallet::CzBCXWallet(std::string strWalletFile)
{
    ...
    //Check for old db version of storing zbcx seed
    if (fFirstRun) {
        uint256 seed;
        if (walletdb.ReadZBCXSeed_deprecated(seed)) {
            //Update to new format, erase old
            seedMaster = seed;
            hashSeed = Hash(seed.begin(), seed.end());
            if (pwalletMain->AddDeterministicSeed(seed)) {
                if (walletdb.EraseZBCXSeed_deprecated()) {
                    LogPrintf("%s: Updated zBCX seed databasing\n", __func__);
                    fFirstRun = false;
                } else {
                    LogPrintf("%s: failed to remove old zbcx seed\n", __func__);
                }
            }
        }
    }
    ...
    if (fFirstRun) {
        ...
        seedMaster = seed;
        LogPrintf("%s: first run of zbcx wallet detected, new seed generated. Seedhash=%s\n", __func__, Hash(seed.begin(), seed.end()).GetHex());
...
bool CzBCXWallet::SetMasterSeed(const uint256& seedMaster, bool fResetCount)
{
    ...
    if (fResetCount)
        walletdb.WriteZBCXCount(nCountLastUsed);
    else if (!walletdb.ReadZBCXCount(nCountLastUsed))
        nCountLastUsed = 0;
...
void CzBCXWallet::Lock()
{
    seedMaster = 0;
}

void CzBCXWallet::AddToMintPool(const std::pair<uint256, uint32_t>& pMint, bool fVerbose)
{
    mintPool.Add(pMint, fVerbose);
}

//Add the next 20 mints to the mint pool
void CzBCXWallet::GenerateMintPool(uint32_t nCountStart, uint32_t nCountEnd)
{
    ...
    for (uint32_t i = n; i < nStop; ++i) {
        ...
        CKey key;
        SeedToZBCX(seedZerocoin, bnValue, bnSerial, bnRandomness, key);
...
bool CzBCXWallet::LoadMintPoolFromDB()
{
...
void CzBCXWallet::RemoveMintsFromPool(const std::vector<uint256>& vPubcoinHashes)
{
    for (const uint256& hash : vPubcoinHashes)
        mintPool.Remove(hash);
}

void CzBCXWallet::GetState(int& nCount, int& nLastGenerated)
{
...
void CzBCXWallet::SyncWithChain(bool fGenerateMintPool)
{
    ...
    while (found) {
        ...
        for (pair<uint256, uint32_t> pMint : listMints) {
            ...
            if (pwalletMain->zbcxTracker->HasPubcoinHash(pMint.first)) {
...
bool CzBCXWallet::SetMintSeen(const CBigNum& bnValue, const int& nHeight, const uint256& txid, const CoinDenomination& denom)
{
    ...
    CBigNum bnRandomness;
    CKey key;
    SeedToZBCX(seedZerocoin, bnValueGen, bnSerial, bnRandomness, key);
    ...
    // Add to zbcxTracker which also adds to database
    pwalletMain->zbcxTracker->Add(dMint, true);
...
void CzBCXWallet::SeedToZBCX(const uint512& seedZerocoin, CBigNum& bnValue, CBigNum& bnSerial, CBigNum& bnRandomness, CKey& key)
{
...
uint512 CzBCXWallet::GetZerocoinSeed(uint32_t n)
{
...
void CzBCXWallet::GenerateDeterministicZBCX(CoinDenomination denom, PrivateCoin& coin, CDeterministicMint& dMint, bool fGenerateOnly)
{
...
void CzBCXWallet::GenerateMint(const uint32_t& nCount, const CoinDenomination denom, PrivateCoin& coin, CDeterministicMint& dMint)
{
...
bool CzBCXWallet::CheckSeed(const CDeterministicMint& dMint)
{
...
bool CzBCXWallet::RegenerateMint(const CDeterministicMint& dMint, CZerocoinMint& mint)
{
    if (!CheckSeed(dMint)) {
        uint256 hashSeed = Hash(seedMaster.begin(), seedMaster.end());
