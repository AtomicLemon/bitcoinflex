...
CzBCXTracker::CzBCXTracker(std::string strWalletFile)
{
...
CzBCXTracker::~CzBCXTracker()
{
    mapSerialHashes.clear();
    mapPendingSpends.clear();
}

void CzBCXTracker::Init()
{
...
bool CzBCXTracker::Archive(CMintMeta& meta)
{
...
bool CzBCXTracker::UnArchive(const uint256& hashPubcoin, bool isDeterministic)
{
    CWalletDB walletdb(strWalletFile);
    if (isDeterministic) {
        CDeterministicMint dMint;
...
CMintMeta CzBCXTracker::Get(const uint256 &hashSerial)
{
...
CMintMeta CzBCXTracker::GetMetaFromPubcoin(const uint256& hashPubcoin)
{
...
bool CzBCXTracker::GetMetaFromStakeHash(const uint256& hashStake, CMintMeta& meta) const
{
    for (auto& it : mapSerialHashes) {
...
CoinWitnessData* CzBCXTracker::GetSpendCache(const uint256& hashStake)
{
...
bool CzBCXTracker::ClearSpendCache()
{
    AssertLockHeld(cs_spendcache);
...
std::vector<uint256> CzBCXTracker::GetSerialHashes()
{
...
CAmount CzBCXTracker::GetBalance(bool fConfirmedOnly, bool fUnconfirmedOnly) const
{
    CAmount nTotal = 0;
...
CAmount CzBCXTracker::GetUnconfirmedBalance() const
{
    return GetBalance(false, true);
}

std::vector<CMintMeta> CzBCXTracker::GetMints(bool fConfirmedOnly) const
{
...
bool CzBCXTracker::HasMintTx(const uint256& txid)
{
    for (auto it : mapSerialHashes) {
...
bool CzBCXTracker::HasPubcoin(const CBigNum &bnValue) const
{
...
bool CzBCXTracker::HasPubcoinHash(const uint256& hashPubcoin) const
{
    for (auto it : mapSerialHashes) {
        CMintMeta meta = it.second;
        if (meta.hashPubcoin == hashPubcoin)
...
bool CzBCXTracker::HasSerial(const CBigNum& bnSerial) const
{
    uint256 hash = GetSerialHash(bnSerial);
    return HasSerialHash(hash);
}

bool CzBCXTracker::HasSerialHash(const uint256& hashSerial) const
{
    auto it = mapSerialHashes.find(hashSerial);
    return it != mapSerialHashes.end();
}

bool CzBCXTracker::UpdateZerocoinMint(const CZerocoinMint& mint)
{
...
bool CzBCXTracker::UpdateState(const CMintMeta& meta)
{
    CWalletDB walletdb(strWalletFile);
...
void CzBCXTracker::Add(const CDeterministicMint& dMint, bool isNew, bool isArchived, CzBCXWallet* zBCXWallet)
{
    bool iszBCXWalletInitialized = (NULL != zBCXWallet);
    CMintMeta meta;
    ...
    if (! iszBCXWalletInitialized)
        zBCXWallet = new CzBCXWallet(strWalletFile);
    meta.isSeedCorrect = zBCXWallet->CheckSeed(dMint);
    if (! iszBCXWalletInitialized)
        delete zBCXWallet;
    mapSerialHashes[meta.hashSerial] = meta;
...
void CzBCXTracker::Add(const CZerocoinMint& mint, bool isNew, bool isArchived)
{
    CMintMeta meta;
...
void CzBCXTracker::SetPubcoinUsed(const uint256& hashPubcoin, const uint256& txid)
{
...
void CzBCXTracker::SetPubcoinNotUsed(const uint256& hashPubcoin)
{
    if (!HasPubcoinHash(hashPubcoin))
        return;
    CMintMeta meta = GetMetaFromPubcoin(hashPubcoin);
...
void CzBCXTracker::RemovePending(const uint256& txid)
{
    uint256 hashSerial;
...
bool CzBCXTracker::UpdateStatusInternal(const std::set<uint256>& setMempool, CMintMeta& mint)
{
...
std::set<CMintMeta> CzBCXTracker::ListMints(bool fUnusedOnly, bool fMatureOnly, bool fUpdateStatus, bool fWrongSeed, bool fExcludeV1)
{
    CWalletDB walletdb(strWalletFile);
    if (fUpdateStatus) {
        ...
        CzBCXWallet* zBCXWallet = new CzBCXWallet(strWalletFile);
        for (auto& dMint : listDeterministicDB) {
            if (fExcludeV1 && dMint.GetVersion() < 2)
                continue;
            Add(dMint, false, false, zBCXWallet);
        }
        delete zBCXWallet;
        LogPrint("zero", "%s: added %d dzbcx from DB\n", __func__, listDeterministicDB.size());
    }
