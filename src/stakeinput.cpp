...
// The zBCX block index is the first appearance of the accumulator checksum that was used in the spend
...
{
    //The unique identifier for a zBCX is a hash of the serial
...
{
    //Create an output returning the zBCX that was staked
    CTxOut outReward;
    libzerocoin::CoinDenomination denomStaked = libzerocoin::AmountToZerocoinDenomination(this->GetValue());
    CDeterministicMint dMint;
    if (!pwallet->CreateZBCXOutPut(denomStaked, outReward, dMint))
        return error("%s: failed to create zBCX output", __func__);
    vout.emplace_back(outReward);

    //Add new staked denom to our wallet
    if (!pwallet->DatabaseMint(dMint))
        return error("%s: failed to database the staked zBCX", __func__);

    for (unsigned int i = 0; i < 3; i++) {
        CTxOut out;
        CDeterministicMint dMintReward;
        if (!pwallet->CreateZBCXOutPut(libzerocoin::CoinDenomination::ZQ_ONE, out, dMintReward))
            return error("%s: failed to create zBCX output", __func__);
...
{
    CzBCXTracker* zbcxTracker = pwallet->zbcxTracker.get();
    CMintMeta meta;
    if (!zbcxTracker->GetMetaFromStakeHash(hashSerial, meta))
        return error("%s: tracker does not have serialhash", __func__);

    zbcxTracker->SetPubcoinUsed(meta.hashPubcoin, txid);
    return true;
}
...
{
    //The unique identifier for a BCX stake is the outpoint
