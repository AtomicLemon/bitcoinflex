...
public:
    ...
    bool WriteZBCXSeed(const uint256& hashSeed, const vector<unsigned char>& seed);
    bool ReadZBCXSeed(const uint256& hashSeed, vector<unsigned char>& seed);
    bool ReadZBCXSeed_deprecated(uint256& seed);
    bool EraseZBCXSeed();
    bool EraseZBCXSeed_deprecated();

    bool WriteZBCXCount(const uint32_t& nCount);
    bool ReadZBCXCount(uint32_t& nCount);
