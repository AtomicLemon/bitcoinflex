...
#include "ui_zbcxcontroldialog.h"
...
void ZPivControlDialog::updateList()
{
    ...
    for (const CMintMeta& mint : setMints) {
        ...
        {
            LOCK(pwalletMain->zbcxTracker->cs_spendcache);

            CoinWitnessData *witnessData = pwalletMain->zbcxTracker->GetSpendCache(mint.hashStake);
        ...
        if(!fSpendable) {
            ...
            if(nConfirmations < Params().Zerocoin_MintRequiredConfirmations())
                ...
                strReason = "Your wallet is locked. Impossible to precompute or spend zBCX.";
            else if (!mint.isSeedCorrect)
                strReason = "The zBCX seed used to mint this zBCX is not the same as currently hold in the wallet";
            else
