...
/**
 * BitcoinFlex RPC command dispatcher.
 */
class CRPCTable
{
private:
...
extern UniValue DoZbcxSpend(const CAmount nAmount, bool fMintChange, bool fMinimizeChange, vector<CZerocoinMint>& vMintsSelected, std::string address_str, bool isPublicSpend = true);
...
extern UniValue setzbcxseed(const UniValue& params, bool fHelp);
extern UniValue getzbcxseed(const UniValue& params, bool fHelp);
extern UniValue generatemintlist(const UniValue& params, bool fHelp);
extern UniValue searchdzbcx(const UniValue& params, bool fHelp);
extern UniValue dzbcxstate(const UniValue& params, bool fHelp);
